...
public class ImageUtils {
    ...
    public static BufferedImage getGrayScaleImage(BufferedImage bimg, float wr,
            float wg, float wb) {

        BufferedImage grey = new BufferedImage(bimg.getWidth(),
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int x = 0; x < w; x++) {
                ...
                int rgb = pack((int) (color * 255), (int) (color * 255),
                        (int) (color * 255));
    ...
    public static int getChannelValue(BufferedImage bimg, int x, int y,
            Channel channel) {
    ...
    public static BinaryImage applyRobertsonEdgeDetection(
            BinaryImage binaryImage) {

        boolean[][] data = new boolean[binaryImage.getWidth()][binaryImage
                .getHeight()];
        ...
        for (int y = 1; y < binaryImage.getHeight() - 1; y++) {
            ...
            for (int x = 1; x < binaryImage.getWidth() - 1; x++) {
                ...
                PixelNeighbours neighbour = new PixelNeighbours(x, y,
                        binaryImage);

                data[x][y] = (binaryImage.getValueAt(x, y) ^ neighbour.cell5)
                        || (neighbour.cell4 ^ neighbour.cell6);

            }

        }

        return new BinaryImage(data, binaryImage.getWidth(),
