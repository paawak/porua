...
public class BasicTesseractExampleTest {
    ...
    private static final String BANGLA_IMAGE_FILE =
	    "/kaaj/source/porua/porua-frontend/image-store/training/bangla/rajshekhar-basu-mahabharat/Bangla-mahabharat-1-page_1/Bangla-mahabharat-1-page_1.png";

    @Test
    public void givenTessBaseApi_whenImageOcrd_thenTextDisplayed_eng() throws Exception {

	// Open input image with leptonica library
	String imageFile = BasicTesseractExampleTest.class
		.getResource("/com/swayam/ocr/porua/res/test-english.png").getPath();
	...
	String expected = readFile(BasicTesseractExampleTest.class
		.getResource("/com/swayam/ocr/porua/res/test-english.txt").getPath());
	...
	// Open input image with leptonica library
	String imageFile = BasicTesseractExampleTest.class
		.getResource("/com/swayam/ocr/porua/res/Bangla-300-short.png").getPath();
	PIX image = pixRead(imageFile);
	BytePointer outText = null;
	...
	api.SetImage(image);
	BOXA boxes =
		api.GetComponentImages(tesseract.RIL_TEXTLINE, true, (PIXA) null, (IntBuffer) null);
	...
	for (int i = 0; i < boxes.n(); i++) {
	    ...
	    System.out.printf("Box[%d]: x=%d, y=%d, w=%d, h=%d, confidence: %d, text: %s%n", i,
		    box.x(), box.y(), box.w(), box.h(), conf, ocrText);
	}
	...
	while (ri.Next(level)) {
	    ...
	    System.out.printf("word: '%s';  \tconf: %.2f; BoundingBox: %d,%d,%d,%d;%n", ocrText,
		    conf, x1.get(), y1.get(), x2.get(), y2.get());
	}
