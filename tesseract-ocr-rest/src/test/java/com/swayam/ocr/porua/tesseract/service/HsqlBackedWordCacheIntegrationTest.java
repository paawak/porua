...
class HsqlBackedWordCacheIntegrationTest {
    ...
    @Test
    void testStoreImageFile() throws SQLException {
	// given

	// when
	testClass.storeImageFile("MyRandomFileForTest.tiff", Language.ben);
	...
	int imageFileId = testClass.storeImageFile("MyRandomFileForTest.tiff", Language.ben);
	rawTexts.forEach(rawText -> testClass.storeRawOcrWord(imageFileId, rawText));
